var Box = {
	...
	shiftRandom: function() {
		var i = 0, tmP = [];
		while(i < this.p.length){
   			tempP = Particle.create((Math.random()*2 - 1/*generatees a random number between -1, 1*/) * 100, (Math.random()*2 - 1) * 100, (Math.random()*2 - 1) * 100, tmP.length);
		    var l = tmP.length;
		    if(l != 0){
		      for(var j = 0; j < l; j+=1){
		        _dist = tempP.distanceTo(tmP[j], this.size);
		        if(_dist < 2*tmP[j].radius){
		          c = c+1;
		        }
		      }
		    }
	...
	calcTotLJPotential: function () {
		for (var i = 0; i < this.p.length; i++) {
			for (var j = 0; j < this.p.length; j++) {
				var x = this.p[i].distanceTo(this.p[j], this.size);
				// console.log("distcance is", this.p[i].xp, this.p[j].xp);
				// console.log("distcance is", x);
				this.p[i].calcPotential(this.p[j], this.sigma, this.epsilon, this.size);
			}
			...
			// this.p[i].display();
			this.TotalPot = this.TotalPot + this.p[i].potential; 

		}
		console.log("The Total Potential of the System:", this.TotalPot);
	},
	acceptance: function(b2) {
		this.p = b2.p;
	},
};
