var Particle = {
  ...
  calcPotential: function(p2, sigma, epsilon, boxLength){
     var dist = 0;
     if(p2.index == this.index)
       { dist = Infinity;}
     else{
     ...
     if (r < ljcut && this.isForceCalculated[this.index][p2.index] == false) {
       xforce =  (4 * epsilon) * ((sigma / r) ** 12 - (sigma/r)**6);
       //(4*eps*((sig/r(i,j))**12 - (sig/r(i,j))**6))
       //yforce =  (48 * epsilon * this._y) * ((sigma / r) ** 12 - 0.5*(sigma/r)**6)/(r**2);
       //zforce =  (48 * epsilon * this._z) * ((sigma / r) ** 12 - 0.5*(sigma/r)**6)/(r**2);
      // console.log("updated accelaration is now", xforce);
       this.isForceCalculated[this.index][p2.index] = true;
    ...
    // console.log("dist annd ljcut", this.isForceCalculated[this.index][p2.index], this.index, p2.index);
      ...
      yforce = 0;
      zforce = 0;
      this.isForceCalculated[this.index][p2.index] = false;
      this.isForceCalculated[p2.index][this.index] = false;
      p2.isForceCalculated[this.index][p2.index] = false;
      p2.isForceCalculated[p2.index][this.index] = false;
    }
   this.potential = this.potential + xforce;
   p2.potential = p2.potential + xforce;
