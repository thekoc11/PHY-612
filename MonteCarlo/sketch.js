...
function setup(){
  ...
  b.calcTotLJPotential();
  bxp = Box.create(200, 50, 0.3, 0.1);//boxlength, noOfParticles, initialTemp, dt
  bxp.calcTotLJPotential();

  var DeltaV = bxp.TotalPot - b.TotalPot;
  if(DeltaV < 0){
    b.acceptance(bxp);
  }
  else if(Math.exp(DeltaV) < Math.random()){
    b.acceptance(bxp);
  }
